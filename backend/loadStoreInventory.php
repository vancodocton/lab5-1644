<?php
	...
	$query = "SELECT p.id, p.name, ci.quantity, ci.time FROM products as p LEFT JOIN (SELECT * FROM StoreInventory WHERE StoreId = ".$_SESSION['storeid'].") AS ci ON ci.productid = p.id ORDER BY p.id;";
